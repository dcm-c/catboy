...
// Ablak generáló (rövidítve a hely miatt)
function createFakeWindow(x, y, t, c) { const w = document.createElement('div'); w.className = 'retro-window'; w.style.left = x + 'px'; w.style.top = y + 'px'; w.style.zIndex = 10000 + Math.floor(Math.random() * 100); w.innerHTML = `<div class="retro-header"><span>${t}</span><div class="retro-close-btn">X</div></div><div class="retro-body">${c}</div>`; w.querySelector('.retro-close-btn').onclick = () => w.remove(); w.onmousedown = () => w.style.zIndex = parseInt(w.style.zIndex) + 100; document.body.appendChild(w); }
